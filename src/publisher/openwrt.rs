...
fn net_line(line: String) -> Option<proto::Netdev> {
    ...
    for addr in &iface.addresses {
        match addr.kind {
            interfaces::Kind::Ipv4 | interfaces::Kind::Ipv6 => {
...
fn switch() -> Option<Vec<proto::Switch>>  {
    ...
    let cmd = trye!(71, Command::new("swconfig")
            .arg("dev")
            .arg("switch0")
            .arg("show")
            .output());
...
pub fn sysinfo(_poll: osaka::Poll, _headers: headers::Headers, _: &identity::Identity, mut stream: endpoint::Stream)
    -> Option<osaka::Task<()>>
{
...
pub fn wifi_interface(i: &str) ->  Option<proto::WifiInterface> {
    ...
    for line in lines {
        ...
        match line.next() {
             ...
             Some("expected throughput") => cur.expected    = {
                 line.next().and_then(|v|{
                     let mut v = v.split_whitespace();
                     v.next().and_then(|v|{
...
fn get_arp() -> Vec<proto::Arp> {
    ...
    for line  in f.lines() {
        ...
        r.push(proto::Arp{
            ip:     line[0].into(),
            hw:     if line[1] == "0x1" {proto::arp::HwType::Ethernet as i32} else {0},
            mac:    line[3].into(),
            dev:    line[5].into(),
        });
...
pub fn netsurvey(poll: osaka::Poll, headers: headers::Headers, identity: &identity::Identity, mut stream: endpoint::Stream)
    -> Option<osaka::Task<()>>
{
...
pub fn ota(poll: osaka::Poll, headers: headers::Headers, _: &identity::Identity, mut stream: endpoint::Stream)
    -> Option<osaka::Task<()>>
{
