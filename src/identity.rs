...
impl Secret {
    pub fn identity(&self) -> Identity {
        ...
        let pk: PublicKey = (&secret_key).into();
        Identity::from_bytes(pk.as_bytes()).unwrap()
    }

    pub fn address(&self) -> Address {
        Address(x25519_dalek::x25519(*self.0.clone(), x25519_dalek::X25519_BASEPOINT_BYTES))
    }
    ...
    pub fn sign(&self, purpose: &[u8], text: &[u8]) -> Signature {
        ...
        Signature(kp.sign(&stext).to_bytes())
    }
...
impl FromStr for Secret {
    ...
    fn from_str(s: &str) -> Result<Self, Self::Err> {
        ...
        if s[1] != 3 {
            return Err(Error::InvalidAddressType{need: address_type_name(3), got: address_type_name(s[1])}.into());
        }
...
impl FromStr for Address {
    ...
    fn from_str(s: &str) -> Result<Self, Self::Err> {
        ...
        if s[1] != 6 {
            return Err(Error::InvalidAddressType{need: address_type_name(6), got: address_type_name(s[1])}.into());
        }
...
impl FromStr for Signature {
    ...
    fn from_str(s: &str) -> Result<Self, Self::Err> {
        ...
        if s[1] != 2 {
            return Err(Error::InvalidAddressType{need: address_type_name(2), got: address_type_name(s[1])}.into());
        }
...
impl FromStr for Identity {
    ...
    fn from_str(s: &str) -> Result<Self, Self::Err> {
        ...
        if s[1] != 9 {
            return Err(Error::InvalidAddressType{need: address_type_name(9), got: address_type_name(s[1])}.into());
        }
...
fn address_type_name(t: u8) -> String {
    match t {
        2 =>"Signature".into(),
        3 =>"Secret".into(),
        6 => "Address".into(),
...
fn sign() {
    ...
    assert!(client_identity
        .verify(b"goes on postcards", text, &signature)
        .is_ok());
    assert!(client_identity
