...
pub type RouteHandler = Box<Fn(
    Poll,
    headers::Headers,
    &identity::Identity,
    endpoint::Stream) -> Option<osaka::Task<()>>>;
...
fn newstreamhandler(
    ...
    let resource = headers.path().as_ref().map(|v|String::from_utf8_lossy(v).to_string()).unwrap_or(String::from(""));
