...
impl QuicRecovery {
    ...
    pub fn on_packet_sent(&mut self, seq: u64, frames: Vec<Frame>, now: u64) {
        let (bytes, ackonly) = frames.iter().fold((0, true), |(bytes, ackonly), frame| {
            ...
            (
                bytes + if frame.is_ack() { 0 } else { frame.len() },
                ackonly,
            )
        });
        ...
        assert!(
            old.is_none(),
            "cannot send packets with packet id already in flight"
        );
    ...
    fn update_rtt(&mut self, ack_delay: u64) {
        ...
        if self.smoothed_rtt == 0 {
            ...
            self.smoothed_rtt =
                (7.0 / 8.0 * self.smoothed_rtt as f64 + 1.0 / 8.0 * self.latest_rtt as f64) as u64;
        }
    }
    ...
    fn set_loss_detection_alarm(&mut self) {
        ...
        let alarm_duration = if self.loss_time != 0 {
            ...
            let mut alarm_duration =
                self.smoothed_rtt + (4.0 * self.rttvar) as u64 + self.max_ack_delay;
        ...
        self.loss_detection_alarm =
            Some(self.time_of_last_sent_retransmittable_packet + alarm_duration);
    }
    ...
    fn on_packet_acked_cc(&mut self, acked_packet: Pkt) {
        ...
        if self.congestion_window < self.ssthresh {
            ...
            self.congestion_window +=
                INITIAL_MSS * acked_packet.bytes as u64 / self.congestion_window;
        }
    }
...
fn low_latency_high_loss() {
    ...
    assert_eq!(loss, LossDetection::None);
    assert_eq!(
        qr.congestion_window, 8822,
        "congestion windows should recover"
    );
}
