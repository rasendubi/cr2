...
impl Channel {
        ...
        for frame in frames {
            ...
            match frame {
                Frame::Header { stream, payload } => {
                    ...
                    trace!("[{}] LD1 currently holding {} streams", self.debug_id, self.streams.len());
                    ...
                    let ordered = self
                        .streams
                        .entry(stream)
                        .or_insert(stream::OrderedStream::new());
    ...
    pub fn progress(&mut self) -> Result<ChannelProgress, Error> {
        ...
        Ok(ChannelProgress::Later(Duration::from_millis(
            self.deadline - now,
        )))
    }
